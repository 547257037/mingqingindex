...
import FinancialRentEdit from '../forms/FinancialRentEditForm'
import FinancialRentAdd from '../forms/FinancialRentAddForm'
...
class RentList extends React.Component {
      ...
      render: (text, record, index) => {
        return (
          <div>
            ...
            <Popconfirm title="确认删除?" onConfirm={() => this.onDelete(record.rentId)}>
              <a href="javascript:;" style={{ marginRight: '10px' }}>删除</a>
            </Popconfirm>
            <a href="javascript:;" style={{ marginRight: '10px' }} onClick={() => this.onModify(record.rentId, index)}>修改</a>
  ...
  getUser(name, pNo) {
    var _this = this;
    axios.get(Base.financialAPI + '/financial-rent/page', {
      params: {
        keyword: name || '',
        pageNo: pNo || 1,
        pageSize: 10
      ...
      if (result.status == 200) {
        ...
        if (d != null) {
          d.map(function (v, i) {
            var json = {};
            json.key = i;
            json.rentId = v.rentId;
            json.vendorName = v.vendorName;
            json.houseAddress = v.houseAddress;
            
            json.landlordPhone = v.landlordPhone; 
            json.landlordidCard = v.landlordidCard; 
            json.rentAmount = v.rentAmount;
            json.payTime = moment(v.payTime).format('YYYY-MM-DD');
            json.theDeposit = v.theDeposit
  ...
  onDelete(id) {
    axios.delete(Base.financialAPI + '/financial-rent/delete', {params:{ financialRentId: id }})
      .then(result => {
