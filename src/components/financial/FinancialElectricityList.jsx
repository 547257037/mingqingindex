...
import { Table, Input, Icon, Button, Popconfirm ,Pagination, DatePicker } from 'antd';
var axios = require('axios');
var moment = require('moment')
var Base = require('../../baseConst.js')
const dateFormat = 'YYYY/MM/DD';
import FinancialElectricityEditForm from '../forms/FinancialElectricityEditForm'
import FinancialElectricityAdd from '../forms/FinancialElectricityAddForm'
...
class ElectricityList extends React.Component {
  constructor(props) {
      ...
      render: (text, record, index) => {
        return (
          <div>
            ...
            <Popconfirm title="确认删除?" onConfirm={() => this.onDelete(record.electricityId)}>
              <a href="javascript:;" style={{ marginRight: '10px' }}>删除</a>
            </Popconfirm>
            <a href="javascript:;" style={{ marginRight: '10px' }} onClick={() => this.onModify(record.electricityId, index)}>修改</a>
  ...
  getUser(name, pNo) {
    var _this = this;
    axios.get(Base.financialAPI + '/financial-electricity/page', {
      params: {
        keyword: name || '',
        pageNo: pNo || 1,
        pageSize: 10
      ...
      if (result.status == 200) {
        ...
        if (d != null) {
          d.map(function (v, i) {
            var json = {};
            json.key = i;
            json.startTime = moment(v.startTime).format('YYYY-MM-DD');
            json.vendorName = v.vendorName;
            json.electricityId = v.electricityId;
            json.endTime = moment(v.endTime).format('YYYY-MM-DD');
            json.price = v.price;
            json.useTotalAmount = v.useTotalAmount; 
            json.payTotalAmount = v.payTotalAmount; 
            json.payTime =moment(v.payTime).format('YYYY-MM-DD');
  ...
  onDelete(id) {
    axios.delete(Base.financialAPI + '/financial-electricity/delete', {params:{ electricityId: id }})
      .then(result => {
