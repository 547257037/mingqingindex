...
class NormalLoginForm extends React.Component {
  ...
  handleSubmit(e){
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        axios.post(Base.userAPI + '/user/login', {
          ...
          password: values.password
            ...
            if (result.status == 200 && result.data.code == 200 ) {
                ...
                window.localStorage.setItem('menuVOS', JSON.stringify(result.data.data.menuVOS))
                message.success('登录成功！！');
                func(true);
                window.location.href = window.location.href + 'app/panel/OrderTotal';
            } else {
