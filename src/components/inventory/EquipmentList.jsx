...
import EquipmentEdit from '../forms/EquipmentEditForm'
import EquipmentAdd from '../forms/EquipmentAddForm'
...
class EquipmentList extends React.Component {
  constructor(props) {
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return (
          <div>
            <Popconfirm title="确认删除?" onConfirm={() => this.onDelete(record.itemId)}>
              <a href="javascript:;" style={{ marginRight: '10px' }}>删除</a>
            </Popconfirm>
            <a href="javascript:;" style={{ marginRight: '10px' }} onClick={() => this.onModify(record.itemId, index)}>修改</a>
          </div>
        );
      },
    }];
  ...
  getidEquipmentTypeName (idTypeCode) {
    var idTypeCodeText;
    switch (idTypeCode) {
      case 0:
        idTypeCodeText = '车床';
  ...
  getUser(name, pNo) {
    var _this = this;
    axios.get(Base.inventoryAPI + '/inventory-equipment/page', {
      params: {
        keyword: name || '',
        pageNo: pNo || 1,
        pageSize: 10
      ...
      if (result.status == 200) {
        ...
        if (d != null) {
          d.map(function (v, i) {
            ...
            json.equipmentUrl = v.equipmentUrl;
            json.equipmentName = v.equipmentName; 
            json.equipmentType = v.equipmentType;//（0 车床 ，1铣床， 2， 数控）
            json.equipmentTypeName = _this.getidEquipmentTypeName(v.equipmentType);//（0 车床 ，1铣床， 2， 数控）

            json.vendorName = v.vendorName;
            json.equipmentPrice = v.equipmentPrice;
            json.vendorId = v.vendorId;

            json.equipmentNumber = v.equipmentNumber;
  ...
  onDelete(id) {
    axios.delete(Base.inventoryAPI + '/inventory-equipment/delete', {params:{ equipmentId: id }})
      .then(result => {
    ...
    return (
      <div>
        <div className="example-input">
          <Input size="large" placeholder="输入设备名称" onChange={this.handleChangeSearchInput.bind(this)} />
