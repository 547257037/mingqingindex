...
import ItemEdit from '../forms/ItemEditForm'
import ItemAdd from '../forms/ItemAddForm'
...
class MaterialList extends React.Component {
  ...
  getUser(name, pNo) {
    var _this = this;
    axios.get(Base.inventoryAPI + '/inventory-item/page', {
      params: {
        keyword: name || '',
        pageNo: pNo || 1,
        pageSize: 10
      ...
      if (result.status == 200) {
        ...
        if (d != null) {
          d.map(function (v, i) {
            ...
            json.itemUrl = v.itemUrl;
            json.vendorId = v.vendorId;
            json.vendorName = v.vendorName;
            json.materialId = v.materialId;
            json.materialName = v.materialName;
            json.itemSpecification = v.itemSpecification;
            json.itemPrice = v.itemPrice;
            json.itemDate = moment(v.itemDate).format('YYYY-MM-DD');
            json.itemNumber = v.itemNumber;
  ...
  onDelete(id) {
    axios.delete(Base.inventoryAPI + '/inventory-material/delete', {params:{ itemId: id }})
      .then(result => {
    ...
    return (
      <div>
        <div className="example-input">
          <Input size="large" placeholder="输入商品名称" onChange={this.handleChangeSearchInput.bind(this)} />
